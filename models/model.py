...
from models.attn import AttentionLayer, FullAttention, ProbAttention
from models.decoder import Decoder, DecoderLayer
from models.embed import DataEmbedding
from models.encoder import ConvLayer, Encoder, EncoderLayer, EncoderStack
...
class InformerStack(nn.Module):
    def __init__(self,
        ...
        inp_lens = list(range(
            len(e_layers)))  # [0,1,2,...] you can customize here
        encoders = [
            Encoder([
                EncoderLayer(AttentionLayer(Attn(
                    False,
                    ...
                    output_attention=output_attention),
                             ...
                             d_ff,
                             dropout=dropout,
                             activation=activation) for _ in range(el)
            ], [ConvLayer(d_model) for _ in range(el - 1)] if distil else None,
                    norm_layer=torch.nn.LayerNorm(d_model)) for el in e_layers
        ]
